...
public class ExponentialMovingAverage {
    ...
    private static Map<String, Object> simulate(int shortWindow, int longWindow) {
        StockDataManager dataManager = new StockDataManager();
        dataManager.loadHistoricalDataFromCSV("stock_data/consolidated_stock_data.csv");
        List<String> stocks = dataManager.getStocks();
    ...
    public static void main(String[] args) {
        ...
        for (int shortWindow = minShortWindow; shortWindow <= maxShortWindow; shortWindow += stepShortWindow) {
            for (int longWindow = minLongWindow; longWindow <= maxLongWindow; longWindow += stepLongWindow) {
                ...
                Map<String, Object> result = simulate(shortWindow, longWindow);
                result.put("ShortWindow", shortWindow);
                result.put("LongWindow", longWindow);
                results.add(result);
            }
        }

        // Save results to CSV
        try (FileWriter writer = new FileWriter("ema_results.csv")) {
            writer.append("ShortWindow,LongWindow,FinalCapital,MaxDrawdown,SharpeRatio\n");
            for (Map<String, Object> result : results) {
                writer.append(String.format("%d,%d,%.2f,%.6f,%.6f\n",
                        result.get("ShortWindow"),
                        result.get("LongWindow"),
                        result.get("FinalCapital"),
                        result.get("MaxDrawdown"),
                        result.get("SharpeRatio")));
            }
            System.out.println("Results saved to ema_results.csv");
        } catch (IOException e) {
